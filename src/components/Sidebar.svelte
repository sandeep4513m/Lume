...
<!-- PREMIUM SIDEBAR -->
<aside
  class="flex flex-col h-full bg-[#f9fafb] dark:bg-[#111822] border-r border-gray-200 dark:border-gray-800 transition-all duration-300 ease-in-out shrink-0 {sessionStore.isSidebarCollapsed
    ? 'w-16'
    : 'w-[280px]'}"
>
  <!-- Logo & Header -->
  <div class="flex items-center justify-between px-3 h-14 shrink-0">
    <div
      class="flex items-center gap-2 overflow-hidden {sessionStore.isSidebarCollapsed
    ...
    >
      ...
      {#if !sessionStore.isSidebarCollapsed}
        <span
    ...
    {#if !sessionStore.isSidebarCollapsed}
      ...
      <button
        onclick={() => sessionStore.setIsSidebarCollapsed(true)}
  ...
  {#if sessionStore.isSidebarCollapsed}
    <button
      onclick={() => sessionStore.setIsSidebarCollapsed(false)}
    ...
    <div class="px-3 mt-4 shrink-0 transition-opacity duration-200">
      <div class="relative flex items-center">
        ...
        <input
          ...
          oninput={(e) => sessionStore.setSearchQuery(e.currentTarget.value)}
        ...
        <button
          onclick={() => {
            sessionStore.setIsBulkSelectMode(!sessionStore.isBulkSelectMode);
            sessionStore.setSelectedSessionIds(new Set());
          }}
          class="ml-2 p-1.5 rounded-lg transition-colors {sessionStore.isBulkSelectMode
  ...
  <div class="flex-1 overflow-y-auto w-full px-3 py-4 space-y-1.5 scroll-smooth relative">
    {#each filteredSessions as session}
      <div
        role="button"
        tabindex="0"
        onclick={() => (sessionStore.isBulkSelectMode ? null : onloadchat(session.id))}
        onkeydown={(e) => {
          if (!sessionStore.isBulkSelectMode && (e.key === "Enter" || e.key === " ")) {
            e.preventDefault();
            onloadchat(session.id);
          }
        }}
        class="group cursor-pointer flex items-center rounded-lg p-2.5 transition-all relative select-none
          {session.id === sessionStore.currentSessionId && !sessionStore.isBulkSelectMode
          ...
          {sessionStore.isSidebarCollapsed ? 'justify-center h-11 w-11 mx-auto border-none p-0' : ''}
          {sessionStore.isBulkSelectMode && sessionStore.selectedSessionIds.has(session.id)
          ? 'bg-emerald-50 dark:bg-emerald-900/10 border-l-2 border-emerald-500'
          : ''}"
        title={session.title}
      >
        {#if sessionStore.isSidebarCollapsed}
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 {session.id === sessionStore.currentSessionId ? 'text-emerald-500' : 'text-gray-400'}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
            ><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path></svg>
        {:else if sessionStore.isBulkSelectMode}
          <!-- Bulk select checkbox mode -->
          <button
            onclick={(e) => {
              e.stopPropagation();
              const s = new Set(sessionStore.selectedSessionIds);
              s.has(session.id) ? s.delete(session.id) : s.add(session.id);
              sessionStore.setSelectedSessionIds(s);
            }}
            class="mr-2.5 shrink-0 w-5 h-5 rounded border-2 flex items-center justify-center transition-colors {sessionStore.selectedSessionIds.has(session.id)
              ? 'bg-emerald-500 border-emerald-500 text-white'
              : 'border-gray-300 dark:border-gray-600 hover:border-emerald-400'}"
          >
            {#if sessionStore.selectedSessionIds.has(session.id)}
          ...
          <div class="flex flex-col flex-1 min-w-0">
            <span class="text-[13px] font-medium truncate {sessionStore.selectedSessionIds.has(session.id) ? 'text-emerald-700 dark:text-emerald-400' : 'text-gray-700 dark:text-gray-300'}">{session.title}</span>
        ...
        {:else}
          <div class="flex flex-col flex-1 min-w-0 pr-14">
            <div class="flex items-center space-x-1.5 min-w-0">
              ...
              {/if}
              {#if sessionStore.editingSessionId === session.id}
                <input
                  value={sessionStore.editingSessionTitle}
                  oninput={(/** @type {any} */ e) => sessionStore.setEditingSessionTitle(e.currentTarget.value)}
                  ...
                  onkeydown={(/** @type {any} */ e) => {
                    ...
                    else if (e.key === "Escape") sessionStore.setEditingSessionId(null);
                  }}
                />
              {:else}
                <span class="text-[13px] truncate leading-tight block pb-0.5 {session.id === sessionStore.currentSessionId ? 'font-medium text-gray-900 dark:text-gray-100' : 'font-normal text-gray-400 dark:text-gray-500'}">{session.title}</span>
              {/if}
            </div>
          ...
          <div class="absolute right-1 flex items-center space-x-0.5 transition-all {sessionStore.activeDropdown === session.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}">
            <!-- Pin button -->
            <button
            ...
            <div class="relative">
              <button
                onclick={(e) => {
                  e.stopPropagation();
                  sessionStore.setActiveDropdown(sessionStore.activeDropdown === session.id ? null : session.id);
                }}
              ...
              {#if sessionStore.activeDropdown === session.id}
                <div
    ...
    {#if filteredSessions.length === 0}
      <div class="text-center text-xs text-gray-400 mt-4">
        {#if sessionStore.isSidebarCollapsed}...{:else}No chats found{/if}
      </div>
    {/if}

    <!-- Bulk Delete floating action bar -->
    {#if sessionStore.isBulkSelectMode && sessionStore.selectedSessionIds.size > 0}
      <div class="sticky bottom-0 left-0 right-0 mx-1 bg-white dark:bg-[#161b22] border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg p-2 flex items-center justify-between">
        <span class="text-[12px] font-medium text-gray-600 dark:text-gray-400 pl-1">{sessionStore.selectedSessionIds.size} selected</span>
        <button
  ...
  <div class="p-3 border-t border-gray-200 dark:border-gray-800 shrink-0 mt-auto relative">
    ...
    {#if sessionStore.isSidebarCollapsed}
      <button onclick={() => sessionStore.setIsSidebarCollapsed(false)} class="w-full flex justify-center py-1" title={userName}>
    ...
    <!-- User Menu Popup -->
    {#if isUserMenuOpen && !sessionStore.isSidebarCollapsed}
